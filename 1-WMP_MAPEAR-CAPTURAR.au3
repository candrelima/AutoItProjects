...
#AutoIt3Wrapper_Outfile_x64=WPM_1-6-2021.exe
...
;-------------------------------------------------------------------------
#Region ; ELEMENTOS
$Form2_1 = GUICreate("WPM_MAPEAR/CAPTURAR", 668, 633, 184, 125, -1, BitOR($WS_EX_APPWINDOW,$WS_EX_TOPMOST,$WS_EX_WINDOWEDGE))
...
$Buttonsair = GUICtrlCreateButton("SAIR", 200, 559, 97, 25, $BS_CENTER)
...
$Listhorariosalvo = GUICtrlCreateList("", 16, 408, 177, 175)
GUICtrlSetFont(-1, 13, 800, 0, "Nachlieli CLM")
GUICtrlSetData(-1, "")
$Labellistahorario = GUICtrlCreateLabel("LISTA DE HORARIO:", 16, 384, 183, 26, $SS_CENTER)
GUICtrlSetFont(-1, 15, 400, 0, "Nachlieli CLM")
$Buttonabrirmapa = GUICtrlCreateButton("ABRIR MAPA", 200, 409, 99, 25)
...
$Inputbrilho = GUICtrlCreateInput("100", 195, 537, 25, 20, BitOR($GUI_SS_DEFAULT_INPUT,$ES_CENTER))
GUICtrlSetTip($Inputbrilho, "Definir brilho entre -255 a 255.")
$Inputcontraste = GUICtrlCreateInput("100", 222, 537, 25, 20, BitOR($GUI_SS_DEFAULT_INPUT,$ES_CENTER))
GUICtrlSetTip($Inputcontraste, "Definir contraste entre -100 a 100.")
$Inputraio = GUICtrlCreateInput("5", 249, 537, 25, 20, BitOR($GUI_SS_DEFAULT_INPUT,$ES_CENTER))
GUICtrlSetTip($Inputraio, "Definir raio entre 0 a 255.")
$Inputgrandeza = GUICtrlCreateInput("50", 276, 537, 25, 20, BitOR($GUI_SS_DEFAULT_INPUT,$ES_CENTER))
GUICtrlSetTip($Inputgrandeza, "Definir grandeza entre 0 a 100.")
$Buttoncapturar = GUICtrlCreateButton("CAPTURAR", 200, 435, 99, 25)
GUICtrlSetTip(-1, "Capturar uma única imagem da hora desejada.")
$Buttonlocalizar1min = GUICtrlCreateButton("1MIN", 200, 460, 99, 25)
GUICtrlSetTip(-1, "Localizar tempos chaves não mapeados.")
$Buttonlocalizar5min = GUICtrlCreateButton("5MIN", 200, 485, 99, 25)
GUICtrlSetTip(-1, "Botão indefinido.")
$Buttonindefinido = GUICtrlCreateButton("INDEFINIDO", 200, 510, 99, 25)
GUICtrlSetTip(-1, "Botão indefinido.")
...
Global $playCot, $leep = 5, $message = "", $arqopen, $status = False, _
		$ans = 0
...
AdlibRegister("_Timer1", 250)

While 1

	$brilho		= Int(GUICtrlRead($Inputbrilho, $FO_READ))
	$contraste	= Int(GUICtrlRead($Inputcontraste, $FO_READ))
	$raio		= Int(GUICtrlRead($Inputraio, $FO_READ))
	$grandeza	= Int(GUICtrlRead($Inputgrandeza, $FO_READ))
...
;~ 					_ArrayDisplay($coordssegundou, "$coordssegundou", "", $ARRAYDISPLAY_COLALIGNCENTER)
		...
		Case $Buttoniniciarcaptura
			...
			ElseIf $playCot And $tipo = "replay" Then
				_IniciarCapturaReplay($playCot)
			Else
				MsgBox(0, "STATUS", "SISTEMA INDEFINIDO (LER MAPRA, PREGÃO OU REPLAY?).", 1)
			EndIf
